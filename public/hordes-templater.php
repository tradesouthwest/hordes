 ...
 */
function hordes_attach_single_template_to_page( $template ) {
    // Post ID
        $post_id = get_the_ID();
     
        // For all other CPT
        if ( get_post_type( $post_id ) != 'hordes' ) {
            return $template;
        }
     
        // Else use custom template
        if ( is_single() || is_tax() || is_search() ) {
            return hordes_single_template_hierarchy( 'single-hordes' );
        }
    } 
    add_filter( 'template_include', 'hordes_attach_single_template_to_page' );
     ...
     * @since 1.0
     */
    function hordes_single_template_hierarchy( $template ) {
        ...
        return apply_filters( 'hordes_single_template_' . $template, $file );
    } 